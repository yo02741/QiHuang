...
function MeridianLine({ curve, index }: { curve: MeridianCurve; index: number }) {
  ...
  useFrame(({ clock }, delta) => {
    ...
    const solidTarget = (active ? 0 : anySelected ? 0.12 : 0.85) * entry.current
    ...
    easing.damp(dashedMat, 'opacity', (active ? 1 : 0) * entry.current, 0.25, delta)
...
export function MeridianLines() {
  ...
  return (
    <group>
      {curves.map((c, i) => (
        <MeridianLine key={`${c.meridianId}-${c.side}`} curve={c} index={i} />
      ))}
    </group>
  )
}
